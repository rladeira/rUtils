...
execute.with.cpu.parallel.backend <- function(task.to.be.executed,
  ...
  switch(Sys.info()[['sysname']],
         Windows = execute.with.windows.cpu.parallel.backend(task.to.be.executed, n.cores, ...),
         Linux   = execute.with.linux.cpu.parallel.backend(task.to.be.executed, n.cores, ...))
}
